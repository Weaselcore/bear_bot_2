...
class SoundBoardCog(commands.GroupCog, name="soundboard"):
        ...
        for file in Path("data/sound_bites").iterdir():
            if file.stem.startswith(current):
                list_of_files.append(
                    app_commands.Choice(name=file.stem, value=file.stem)
                )
                count += 1
    ...
    async def stop(self, interaction: Interaction):
        ...
        if interaction.user.voice is None:  # type: ignore
            ...
            await interaction.response.send_message(
                content="Bearbot is not connected to a voice channel.", ephemeral=True
            )

    @app_commands.command(
        description="Create soundbite from Streamable URL", name="streamable"
    )
    async def streamable(self, interaction: Interaction, url: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                ...
                video_url = (
                    response_text.split(url_patterns[0])[1].split(url_patterns[1])[0]
                    if url_patterns[0] in response_text
                    else None
                )
                name_patterns = "/video/mp4/", "?"
                file_name = (
                    response_text.split(name_patterns[0])[1].split(name_patterns[1])[0]
                    if name_patterns[0] in response_text
                    else "video.mp4"
                )
